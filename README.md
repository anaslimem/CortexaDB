# AgentLite: SQLite for AI Agents

[![Build](https://github.com/anaslimem/AgentLite/actions/workflows/rust.yml/badge.svg)](https://github.com/anaslimem/AgentLite/actions/workflows/rust.yml)
[![License: GPL-3.0](https://img.shields.io/badge/License-GPL--3.0-blue.svg)](LICENSE)
[![Status: Beta](https://img.shields.io/badge/Status-Beta-brightgreen.svg)](#current-status)

**AgentLite** is a simple, fast, and hard-durable embedded database designed specifically for AI agent memory. It provides a single-file-like experience (no server required) but with native support for vectors, graphs, and temporal search.
...
### Python (Recommended)
AgentLite is designed to be extremely easy to use from Python via high-performance Rust bindings.
...
db = AgentLite.open("agent.mem", dimension=1536)
...
### Python
AgentLite is available on PyPI and can be installed via `pip`:
...
pip install "agentlite @ git+https://github.com/anaslimem/AgentLite.git#subdirectory=crates/agentlite-py"
```

### Rust
Add AgentLite to your `Cargo.toml`:
```toml
[dependencies]
agentlite-core = { git = "https://github.com/anaslimem/AgentLite.git" }
```
...
- **Automatic Capacity Management**: Set `max_entries` or `max_bytes` and let AgentLite handle LRU/Importance-based eviction automatically.
...
| Method | Description |
|--------|-------------|
| `AgentLite.open(path, ...)` | Opens or creates a database at the specified path. |
...
When calling `AgentLite.open()`, you can tune the behavior:
...
### Why Rust?
AgentLite is written in Rust to provide **memory safety without a garbage collector**, ensuring predictable performance (sub-100ms startup) and low resource overhead—critical for "embedded" use cases where the DB runs inside your agent's process.

### The Storage Engine
AgentLite follows a **Log-Structured** design:
...
### Hybrid Query Engine
Unlike standard vector DBs, AgentLite doesn't just look at distance. Our query planner can:
...
We use a custom versioned serialization layer (with a "magic-byte" header). This allows us to update the AgentLite engine without breaking your existing database files—it knows how to read "legacy" data while writing new records in the latest format.
...
##  License & Status
AgentLite is currently in **Beta (v0.1)**. It is released under the **GPL-3.0 License**.  
We are actively refining the API and welcome feedback!

---
> *AgentLite — Because agents shouldn't have to choose between speed and a soul (memory).*
